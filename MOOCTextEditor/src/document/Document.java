...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		{
			...
			if (count > 1 && word.charAt(word.length() - 1) == 'e' && lastToken.equals("e")) { // compare
																								// to
																								// "e"
																								// not
																								// to
	...
	public double getFleschScore() {
		...
		// according to lecture Flesch score = 206.835 -1.015(words/sentences)-
		// 84.6(syllabus/words)

		float constant1 = 206.835f;
		float constant2 = -1.015f;
		float constant3 = -84.6f;

		float score_1 = getNumWords() / getNumSentences();
		float score_2 = getNumSyllables() / getNumWords();

		float score = constant1 - constant2 * (score_1) - constant3 * (score_2);
